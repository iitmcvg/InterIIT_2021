     ...
     for i in range(NUM_CLASSES):
         ...
         if i == 35 or i == 38 or i == 39:  #### not rotating classes 35,38,39
             transform_list = {"shift": 1, 'scale': 1, 'rotate': 0, 'brightness': 1, 'contrast': 1,
                               'motion_blur': 1, 'rain': 1, 'fog': 1}
         else:
             transform_list = {"shift": 1, 'scale': 1, 'rotate': 1, 'brightness': 1, 'contrast': 1,
                               'motion_blur': 1, 'rain': 1, 'fog': 1}

         transfrom_vals = {"shift_factor": 0.2, "scale_factor": 0.2, "rot_angle": 60, "brightness_factor": 0.1,
                           "contrast_factor": 0.2, "motion_blur_factor": 4, "fog_factor": 0.4, "rain_factor": 3}

         aug = augmentation(class_id=i, dict_transform=transform_list, transform_vals=transfrom_vals,
...
def augment_class(class_id):
    ...
    transfrom_vals = {"shift_factor": 0.2, "scale_factor": 0.2, "rot_angle": 30, "brightness_factor": 0.3,
                      "contrast_factor": 0.2, "motion_blur_factor": 4, "fog_factor": 0.4, "rain_factor": 3}

    aug = augmentation(class_id=class_id, dict_transform=transform_list, transform_vals=transfrom_vals, num_target_imgs=4000)
    aug.load_data()
    aug.setup_pipeline()
    imgs = aug.augment(save=False)
    visualize(imgs=imgs,class_id=class_id)

   

augment_class(np.random.randint(0,42))
